...
`plugins` 是 FlowCloudAI 官方 `.fcplug` 示例插件仓库，聚合 LLM、图像与 TTS 示例实现。  
仓库用于统一验证 manifest、WASM 构建、与客户端加载链路的一致性。
...
该仓库无统一 lint 命令，构建产物与 manifest 一致性作为最小验收指标。

## 代码风格与命名约定

- Rust 使用 2024 Edition，接口与 `tool_fcplug`、`core_ai_client` 语义对齐。  
- 插件 crate 名称与目录大小写保持稳定，避免跨平台解析问题。  
- manifest 与运行时入口需随实现更新保持同步。  
...
- PR 说明需写清每个插件的构建结果、兼容性和回归结论。  
- 目录名或大小写变更需补充跨平台影响说明。  

## 项目特有坑点

- `fcplug-qwen_llm` 与 `fcplug-qwen-tts` 对大小写敏感，跨平台需同步验证。  
- Linux/macOS 严格大小写环境下路径差异容易导致加载失败。  
